...
Primus.prototype.ark["channels"] = function (primus, options) {
      primus.unsubscribe = function(channel) {
        return primus.write({
          action: 'unsubscribe',
          channel: channel
        });
      };
      return primus.subscribe = function(channel, token) {
        var subscribe;
        if (channel == null) {
          channel = null;
        }
        ...
        subscribe = function(channel, token) {
          return primus.write({
            action: 'subscribe',
            channel: channel,
            token: token
        ...
        if ((token != null) || ((channel != null) && !channel.match(options.privatePattern || /^(private|presence)\-/))) {
          return subscribe(channel, token);
        } else {
          return $.post(options.auth_url, {
            channel: channel
          }, function(response) {
            response = JSON.parse(response);
            return subscribe(response.channel, response.token);
          });
        }
      };
    }
